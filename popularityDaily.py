...
def main():
  ...
  before= dt.today() - timedelta(days=1)
  after = dt.today() - timedelta(days=2)
  ...
  try:
      with open('data/resultsDay.txt', 'w') as f:
          for line in resultPop:
              f.write(line[0]+ " - " + str(line[1]) + " - " + str(line[2]) + "\n")
      print("Saved resultsDay.txt file")
  except Error:
  ...
  mycursor.execute("DELETE FROM villagerPopularityDay;")
  mydb.commit()

  sql = "INSERT INTO villagerPopularityDay (villager, postcount, difference) VALUES (%s, %s, %s)"
  try:
